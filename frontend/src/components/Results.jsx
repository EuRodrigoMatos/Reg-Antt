...
function BadgeClasse({ classe }) {
  const cores = {
    ...
    '2.3': 'bg-slate-100 text-slate-800 border border-slate-200',
    ...
    '6.2': 'bg-slate-100 text-slate-800',
    '7':   'bg-yellow-100 text-yellow-800',
    '8':   'bg-slate-100 text-slate-800',
    '9':   'bg-slate-100 text-slate-700',
  }
  return (
    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${cores[classe] || 'bg-slate-100 text-slate-700'}`}>
      Classe {classe}
    </span>
  )
}
    ...
    <div className="border border-slate-200 rounded-lg p-4 bg-white">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="font-semibold text-slate-900">{produto.nomeOficial}</p>
          <p className="text-xs text-slate-700/60 mt-0.5">
            {produto.categoria} · {produto.estadoFisico === 'L' ? 'Líquido' : produto.estadoFisico === 'S' ? 'Sólido' : 'Gás'}
          </p>
          {produto.observacoes && (
            <p className="text-xs text-slate-700 mt-2 bg-slate-50 rounded p-2">{produto.observacoes}</p>
          )}
          ...
          {produto.quantidadeTransportada > 0 && (
            <p className="text-xs text-slate-700/60 mt-1">
  ...
  return (
    <div>
      {rotulos.principais.length > 0 && (
        <div className="mb-4">
          <p className="text-xs font-semibold text-slate-700/60 uppercase tracking-wide mb-3">Rótulos de Risco Obrigatórios</p>
      ...
      {rotulos.subsidiarios.length > 0 && (
        <div className="mt-4">
          <p className="text-xs font-semibold text-slate-700/60 uppercase tracking-wide mb-3">Rótulos Subsidiários</p>
          <div className="flex flex-wrap gap-8 justify-center sm:justify-start">
    ...
    <div className="space-y-1">
      {normas.map((n, i) => (
        <div key={i} className="flex items-center gap-2 text-sm text-slate-800">
          <FileText size={14} className="text-slate-200 shrink-0" />
          {n}
        </div>
      ))}
    </div>
    ...
    <div className="space-y-2">
      ...
      {normais.length > 0 && (
        <div className="pt-2 border-t border-slate-100">
          <p className="text-xs text-slate-700/50 mb-2">{normais.length} produto(s) sem classificação perigosa</p>
          {normais.map((p, i) => (
            <div key={i} className="flex items-center gap-2 p-2 text-sm text-slate-700">
              <CheckCircle size={14} className="text-green-500 shrink-0" />
              {p.nome}
              {p.ncm && <span className="text-xs text-slate-700/50">NCM: {p.ncm}</span>}
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div className="space-y-4 animate-slide-in">
      <div className="flex items-center justify-between">
        ...
        <button onClick={onLimpar} className="text-slate-200 hover:text-slate-700 transition-colors p-1 rounded">
          <X size={20} />
        </button>
      </div>
      ...
      {tipo === 'nfe' && dados && (
        <div className="card">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4 text-sm">
            {dados.nfe?.numero && (
              <div>
                <p className="text-xs text-slate-700/60 font-semibold uppercase tracking-wide">Nº NF</p>
                <p className="font-bold text-slate-900">{dados.nfe.numero}</p>
              </div>
            )}
            {dados.nfe?.emitente?.nome && (
              <div>
                <p className="text-xs text-slate-700/60 font-semibold uppercase tracking-wide">Emitente</p>
                <p className="font-semibold truncate text-slate-900">{dados.nfe.emitente.nome}</p>
              </div>
            )}
            {dados.nfe?.destinatario?.nome && (
              <div>
                <p className="text-xs text-slate-700/60 font-semibold uppercase tracking-wide">Destinatário</p>
                <p className="font-semibold truncate text-slate-900">{dados.nfe.destinatario.nome}</p>
              </div>
            )}
            <div>
              <p className="text-xs text-slate-700/60 font-semibold uppercase tracking-wide">Produtos Perigosos</p>
      ...
      {analiseData && (
        <>
          ...
          <div className="card">
            <h3 className="font-semibold mb-4 text-slate-900">Rótulos de Risco (ABNT NBR 7500)</h3>
            <RotulosSection rotulos={analiseData.rotulos} />
          </div>

          {analiseData.produtos?.length > 0 && (
            <div className="card">
              <h3 className="font-semibold mb-4 text-slate-900">Produtos Perigosos Identificados</h3>
          ...
          {tipo === 'produto-unico' && produto && (
            <div className="card">
              <h3 className="font-semibold mb-4 text-slate-900">Detalhes do Produto</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
                <div>
                  <p className="text-xs text-slate-700/60 font-semibold uppercase tracking-wide">Número ONU</p>
                  <p className="font-bold text-orange-600 text-base">{produto.onu}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-700/60 font-semibold uppercase tracking-wide">Classe de Risco</p>
                  <p className="font-semibold text-slate-900">{produto.classe}{produto.subclasse ? ` + ${produto.subclasse}` : ''}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-700/60 font-semibold uppercase tracking-wide">Número de Risco</p>
                  <p className="font-bold text-slate-900">{produto.numeroRisco}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-700/60 font-semibold uppercase tracking-wide">Grupo de Embalagem</p>
                  <p className="font-semibold text-slate-900">{produto.grupoEmbalagem || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-700/60 font-semibold uppercase tracking-wide">Estado Físico</p>
                  <p className="font-semibold text-slate-900">{produto.estadoFisico === 'L' ? 'Líquido' : produto.estadoFisico === 'S' ? 'Sólido' : 'Gás'}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-700/60 font-semibold uppercase tracking-wide">Qtd. Limitada</p>
                  <p className="font-semibold text-slate-900">{produto.quantidadeLimitada || '—'}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-700/60 font-semibold uppercase tracking-wide">Ficha de Emergência</p>
                  <p className="font-semibold text-slate-900">{produto.fichaEmergencia || '—'}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-700/60 font-semibold uppercase tracking-wide">Categoria</p>
                  <p className="font-semibold text-slate-900">{produto.categoria}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-700/60 font-semibold uppercase tracking-wide">MOPP Exigido</p>
